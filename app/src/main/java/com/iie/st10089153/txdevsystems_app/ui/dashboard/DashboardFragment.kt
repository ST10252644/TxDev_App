...
class DashboardFragment : Fragment() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.action_view_reports -> {
                currentImei?.let { imei ->
                    val bundle = Bundle().apply { putString("IMEI", imei) }
                    findNavController().navigate(R.id.action_dashboard_to_reports, bundle)
                } ?: run {
                    Log.e("DashboardFragment", "Cannot open Reports: IMEI is null")
                }
                true
            }
            else -> super.onOptionsItemSelected(item) // make the when exhaustive
        }
    }
    ...
    /**
     * Navigate to the specified chart destination and pass the IMEI in the bundle.
     * chartDestination should be the destination id (e.g. R.id.navigation_temperature_chart).
     */
    private fun navigateToChart(chartDestination: Int) {
        currentImei?.let { imei ->
            val bundle = Bundle().apply { putString("IMEI", imei) }
            // navigate directly to the chart destination with the IMEI bundle
