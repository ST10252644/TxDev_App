...
class SectionedDeviceAdapter(private val items: List<SectionItem>) :
                ...
                if (rangeResponse.isSuccessful && !rangeResponse.body().isNullOrEmpty()) {
                    ...
                    if (latestRangePoint != null) {
                        ...
                        if (tempMin != null && tempMax != null && tempNow != null) {
                            ...
                            val colorRes = when {
                                !isOnline -> R.color.orange_offline // Offline devices
                                tempNow < tempMin -> R.color.blue_cool // Too cold - BLUE
                                tempNow > tempMax -> R.color.red_error // Too hot - RED
                                else -> R.color.primary // Within range - GREEN
                            }
                            ...
                            android.util.Log.d(
                                "TemperatureRange",
                                "IMEI: $imei, Current: $tempNow°C, Range: $tempMin-$tempMax°C, " +
                                        "Status: ${when {
                                            !isOnline -> "Offline"
                                            tempNow < tempMin -> "Too Cold"
                                            tempNow > tempMax -> "Too Hot"
                                            else -> "Normal"
                                        }}"
                            )
                        } else {
    ...
    private fun fallbackTemperatureColor(
        ...
        // Fallback: Green for online, orange for offline
        val colorRes = if (isOnline) R.color.primary else R.color.orange_offline
        holder.deviceTemp.setTextColor(ContextCompat.getColor(context, colorRes))
    }
