...
import com.iie.st10089153.txdevsystems_app.network.TokenInterceptor
import com.iie.st10089153.txdevsystems_app.network.Api.ReportApi
...
object RetrofitClient {
   //private const val BASE_URL = "http://192.168.5.5:30082/" //just Cherika use this
    ...
    fun getDeviceApi(context: Context): DeviceApi =
        getInstance(context).create(DeviceApi::class.java)

    // Add this method to your RetrofitClient object
    fun getAccountApi(context: Context): AccountApi =
        getInstance(context).create(AccountApi::class.java)


    fun getReportApi(context: Context): ReportApi =
        getInstance(context).create(ReportApi::class.java)
}
