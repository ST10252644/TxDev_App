...
class DoorChartFragmentTest {

    @Test
    fun fragment_displaysUI() {
        // Minimal arguments if your fragment expects any
        val bundle = Bundle().apply { putString("IMEI", "123456789012345") }

        // Launch the fragment with your app theme
        launchFragmentInContainer<DoorChartFragment>(
            fragmentArgs = bundle,
        ...
        onView(withId(R.id.lineChartDoor)).check(matches(isDisplayed()))
        onView(withId(R.id.btnSelectDateDoor)).check(matches(isDisplayed()))
